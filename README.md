...
For high-cardinality workloads, normalization selects the compiled streaming projector (`compiled-bounded-standard-payload`). It writes dictionary fragments, refs, machine and CVE enrichment joins, inventory references, and vulnerability rows through UTF-8 and gzip streams; preserves the production observed-window merge; releases completed lookup collections before payload assembly; and reports working-set, private-memory, and GC-heap checkpoints. Lower-cardinality workloads continue through the PowerShell compatibility normalizer.
...
- Published GitHub releases receive a release asset named `Azure-YYMMDD.zip` from the release tag.

...
| Experiment | Fresh-export machine `stateHash` index + direct-merge fallback | Hosted fresh export | `341.9 MB` | `229.1 MB` | `148.5 MB` | `104.74s` | Replacing the full existing machine current-record map with a compact `id -> stateHash` index cut the live fallback lane by `165.8 MB` WS / `222.9 MB` private / `2.7 MB` GC / `138.44 s` versus the first fallback run. The direct-merge order guard still tripped, but export-stage memory no longer dominated and the peak moved back to `NormalizeDashboardData`. |
...
- Date captured: `2026-05-10` for the fresh-export hosted machine `stateHash` index validation with the targeted Azure fallback still enabled.

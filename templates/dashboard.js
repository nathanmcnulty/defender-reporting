...
function syncFilterStateFromDom() {
    ...
    nextState.deviceNames = getSelectedCheckboxValues('filterDeviceName');
    nextState.rbacGroups = getSelectedCheckboxValues('filterRbacGroup');
    nextState.deviceTags = getSelectedCheckboxValues('filterDeviceTags');
    ...
    nextState.hasDeviceNames = hasAnyChecked('filterDeviceName');
...
function matchesFilterStateNonDate(v, state = filterState) {
    if (state.deviceNameSet.size > 0 && !state.deviceNameSet.has(v.DeviceName)) return false;
